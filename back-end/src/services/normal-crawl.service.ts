...
export async function normalCrawlService(urlWebsite: string, websiteId: number, schema: { [key: string]: ISchemaWebsiteCheerio }) {
    ...
    if (rows.length > 0) {
        ...
        rs && rs.length > 0 && void createOrUpdateConferences(rs);
    }
}
